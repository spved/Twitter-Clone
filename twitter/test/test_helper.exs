defmodule Twitter.Test.Helper do
  ...
  def addUsers(engine, clients) do
    passwd = "passwd"
    email = "user@abc.com"
    Enum.each(0..4, fn count ->
      IO.inspect Enum.at(clients,count), label: count
      Twitter.Engine.insertUser(engine, Enum.at(clients,count), to_string(count), passwd, email)
    end)
  ...
  def addSubscribers() do
    ...
    :ets.insert_new(:subscribers, {"3", ["0", "1", "4"]})
  ...
  def addSubscribersOf() do
    ...
    :ets.insert_new(:subscribedTo, {"4", ["2", "1", "3"]})
